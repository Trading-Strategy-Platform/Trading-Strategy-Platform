...
// GetAll retrieves all indicators with optional category filter using get_indicators function
func (r *IndicatorRepository) GetAll(ctx context.Context, category *string, page, limit int) ([]model.TechnicalIndicator, int, error) {
	...
	err := r.db.SelectContext(ctx, &allIndicators, query, params...)
	if err != nil {
...
// GetByID retrieves an indicator by ID using get_indicator_by_id function
