...
// UploadStrategyThumbnail uploads a thumbnail for a strategy
func (c *MediaClient) UploadStrategyThumbnail(ctx context.Context, strategyID int, fileContent []byte, filename, contentType string) (*MediaFile, error) {
	...
	// Add form fields
	if err := writer.WriteField("purpose", "strategy"); err != nil {
		return nil, fmt.Errorf("failed to write purpose field: %w", err)
	}
	if err := writer.WriteField("entity_id", fmt.Sprintf("%d", strategyID)); err != nil {
