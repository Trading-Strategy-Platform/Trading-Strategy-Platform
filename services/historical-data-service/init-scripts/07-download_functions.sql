...
-- Create a view to display backtest summary
...
-- Function to get backtests with filtering, sorting and pagination
CREATE OR REPLACE FUNCTION get_backtests(
    p_user_id INT,
    p_search VARCHAR DEFAULT NULL,
    p_status VARCHAR[] DEFAULT NULL,
    p_strategy_id INT DEFAULT NULL,
...
BEGIN
    ...
    WHERE 
        b.user_id = p_user_id
        AND (p_search IS NULL OR bs.name ILIKE '%' || p_search || '%')
        AND (p_status IS NULL OR array_length(p_status, 1) IS NULL OR bs.status = ANY(p_status))
        AND (p_strategy_id IS NULL OR bs.strategy_id = p_strategy_id)
        AND (p_start_date IS NULL OR bs.date >= p_start_date)
        AND (p_end_date IS NULL OR bs.date <= p_end_date)
    ORDER BY
...
-- Count backtests with filtering for pagination
...
BEGIN
    ...
    WHERE 
        ...
        AND (p_strategy_id IS NULL OR b.strategy_id = p_strategy_id)
...
-- Function to get backtest by ID
CREATE OR REPLACE FUNCTION get_backtest_by_id(
    p_backtest_id INT,
...
-- Update backtest (add new function for updating backtest details)
CREATE OR REPLACE FUNCTION update_backtest(
    p_backtest_id INT,
    p_user_id INT,
...
-- Count backtest trades for pagination
...
BEGIN
    ...
    WHERE t.backtest_run_id = p_backtest_run_id;
